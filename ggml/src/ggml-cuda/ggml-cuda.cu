...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (ggml_cuda_mach1_skinny_mm(ctx, src0, src1, dst)) {
        return;
    }
...
// --- GGML_MACH1_TIME >= 2: measurement-only per-op timing (see the dispatch
// loop). File-scope so the event drain after the node loop shares the table.
static int m1_op_time_mode() {
    static const int mode = [](){
        const char * s = getenv("GGML_MACH1_TIME");
        return s != nullptr ? atoi(s) : 0;
    }();
    return mode;
}

struct m1_op_time_table {
    std::map<std::string, std::pair<long long, double>> acc;
    std::mutex mtx;
    ~m1_op_time_table() {
        double tot = 0.0;
        for (const auto & kv : acc) tot += kv.second.second;
        fprintf(stderr, "cuda-op-time: TOTAL %.1f ms\n", tot/1000.0);
        for (const auto & kv : acc) {
            fprintf(stderr, "cuda-op-time: %-32s calls=%9lld total=%12.1f us avg=%8.2f us\n",
...
static m1_op_time_table & m1_op_times() {
    static m1_op_time_table t;
    return t;
}

// Bridge so matchers in mach1.cu can record diagnostics here: this table
// prints, mach1.cu's does not, and matcher stderr is filtered by every lane.
void mach1_diag_note(const char * key);

static void m1_op_time_add(const std::string & key, double us) {
    m1_op_time_table & t = m1_op_times();
...
void mach1_diag_note(const char * key) {
    if (m1_op_time_mode() >= 2) {
        m1_op_time_add(std::string(key), 1.0);
    }
}

struct m1_op_evt_pool {
    std::vector<cudaEvent_t> ev;     // 2 per slot
    std::vector<std::string> keys;
    int used = 0;
};

static m1_op_evt_pool & m1_op_evts() {
    static m1_op_evt_pool p;
    return p;
}

static int m1_op_evt_begin(const std::string & key, cudaStream_t stream) {
    m1_op_evt_pool & p = m1_op_evts();
    const int slot = p.used++;
    if ((int) p.keys.size() <= slot) {
        p.keys.resize(slot + 1);
        p.ev.resize(2*slot + 2);
        CUDA_CHECK(cudaEventCreate(&p.ev[2*slot]));
        CUDA_CHECK(cudaEventCreate(&p.ev[2*slot + 1]));
    }
    p.keys[slot] = key;
    CUDA_CHECK(cudaEventRecord(p.ev[2*slot], stream));
    return slot;
}

static void m1_op_evt_end(const int slot, cudaStream_t stream) {
    CUDA_CHECK(cudaEventRecord(m1_op_evts().ev[2*slot + 1], stream));
}

static void m1_op_evt_drain(cudaStream_t stream) {
    m1_op_evt_pool & p = m1_op_evts();
    if (p.used == 0) {
        return;
    }
    CUDA_CHECK(cudaStreamSynchronize(stream));
    for (int s = 0; s < p.used; ++s) {
        float ms = 0.0f;
        CUDA_CHECK(cudaEventElapsedTime(&ms, p.ev[2*s], p.ev[2*s + 1]));
        m1_op_time_add(p.keys[s], (double) ms*1000.0);
    }
    p.used = 0;
}
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_MACH1_NE_MM:
            ggml_cuda_op_mach1_ne_mm(ctx, dst);
            break;
        case GGML_OP_MACH1_EMBED_ROWS:
            ggml_cuda_op_mach1_embed_rows(ctx, dst);
            break;
        case GGML_OP_MACH1_EXP_MM:
            ggml_cuda_op_mach1_exp_mm(ctx, dst);
            break;
        case GGML_OP_MACH1_EXP_BASIS:
            ggml_cuda_op_mach1_exp_basis(ctx, dst);
            break;
        case GGML_OP_MACH1_RT_MM:
            ggml_cuda_op_mach1_rt_mm(ctx, dst);
            break;
        case GGML_OP_MACH1_HEAD_MM:
            ggml_cuda_op_mach1_head_mm(ctx, dst);
            break;
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // Fold-tier mach1 graphs measured FASTER without CUDA graph replay:
        // paired H200, fold 80.8 vs fold+DISABLE_GRAPHS 101.0 tok/s tg128 -
        // replay of the ~970-node decode graph costs 10.9 ms GPU (TIME=5)
        // while direct execution pipelines the same kernels in ~9.9 ms wall.
        // GGML_MACH1_FOLD_GRAPHS=1 restores replay for A/B.
        switch (node->op) {
            case GGML_OP_MACH1_NE_MM:
            case GGML_OP_MACH1_EMBED_ROWS:
            case GGML_OP_MACH1_EXP_MM:
            ...
            case GGML_OP_MACH1_HEAD_MM:
            case GGML_OP_MACH1_EMBED_GATHER: {
                static const bool fold_no_graphs = [](){
                    const char * f = getenv("GGML_MACH1_FOLD");
                    const char * k = getenv("GGML_MACH1_FOLD_GRAPHS");
                    return f != nullptr && atoi(f) != 0 && (k == nullptr || atoi(k) == 0);
                }();
                if (fold_no_graphs) {
        ...
        // the grouped-GEMM expert prefill (GGML_MACH1_EXP_APPLY_GG, n_tok >=
        // 256 only) fetches the group sizes to the host, so its cgraphs
        // cannot capture; decode-shape graphs are unaffected
        if (node->op == GGML_OP_MACH1_EXP_MM) {
            static const bool exp_gg = [](){
                const char * s = getenv("GGML_MACH1_EXP_APPLY_GG");
                return s != nullptr && atoi(s) != 0;
            }();
            if (exp_gg && node->src[6] != nullptr && node->src[6]->ne[1] >= 256) {
                use_cuda_graph = false;
            }
        }
...
static bool ggml_cuda_graph_update_required(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (res || memcmp(&graph->node_props[i], &prop, sizeof(prop)) != 0) {
            // GGML_MACH1_TIME=4: name the first nodes whose properties flip
            // between evals - each flip resets graph warmup to direct execution
            if (m1_op_time_mode() >= 4 && !res) {
                const ggml_cuda_graph::node_properties & old = graph->node_props[i];
                const char * what =
                    old.node.data != prop.node.data ? "data" :
                    memcmp(old.node.op_params, prop.node.op_params, sizeof(prop.node.op_params)) != 0 ? "op_params" :
                    memcmp(old.node.ne, prop.node.ne, sizeof(prop.node.ne)) != 0 ? "ne" :
                    memcmp(old.node_src_data_ptrs, prop.node_src_data_ptrs, sizeof(prop.node_src_data_ptrs)) != 0 ? "src_data" :
                    "other";
                static std::atomic<int> logged{0};
                if (logged.fetch_add(1) < 24) {
                    fprintf(stderr, "graph-prop-flip: node %d %s (%s) field=%s\n",
                            i, ggml_op_name(prop.node.op), prop.node.name, what);
                }
                m1_op_time_add(std::string("graphflip:") + ggml_op_name(prop.node.op) + ":" + what, 1.0);
            }
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    if (node->op == GGML_OP_MACH1_EXP_MM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 expert ffn for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            if (m1_op_time_mode() >= 4) {
                m1_op_time_add("expffn:fused", 1.0);
            }
            return nodes_to_skip;
        }
        // A fused region never reaches the per-op timer, so a matcher that
        // silently stops matching is indistinguishable from one that was
        // never enabled.  Name the guard that rejected it.
        if (m1_op_time_mode() >= 4) {
            m1_op_time_add("expffn:nofuse:line"
                           + std::to_string(ggml_cuda_mach1_exp_ffn_reject_line()), 1.0);
        }
    }
    ...
    if (node->op == GGML_OP_MACH1_RT_MM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 shared-expert ffn for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    if (node->op == GGML_OP_MACH1_RT_MM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 v_tiled reorder for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    if (node->op == GGML_OP_CONT && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 xperm reorder for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    if (node->op == GGML_OP_MACH1_RT_MM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: batched mach1 qkv trio for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    // mach1 rt tail fold: the elementwise consumers of a standalone rt op (the
    // residual add, or the shared-expert sigmoid-gate mul plus the moe and
    ...
    if (node->op == GGML_OP_MACH1_RT_MM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: folded mach1 rt tail for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    // Measurement only (GGML_MACH1_TIME >= 2): record what the level-3 matcher
    // actually sees at a projection mul_mat. Its guards require both weights
    // F32/F16 and one of two positional window forms (19 and 17 nodes), and it
    // never fires on this model - debug fprintf from inside the matcher does
    // not survive the bench lanes' stderr filters, but this table prints, so
    // the diagnosis goes here.
    if (m1_op_time_mode() >= 2 && node->op == GGML_OP_MUL_MAT && node->ne[0] == 32 &&
        node->src[0] != nullptr) {
        ...
        // The glue window the matcher checks positionally (it tries two forms,
        // 19 and 17 nodes), as the graph ACTUALLY emits it. The expected forms
        // are a round-249 reconstruction and the graph has since moved; this
        // prints the larger window so the forms can be corrected rather than
        // guessed.
        if (i + 1 < cgraph->n_nodes && cgraph->nodes[i + 1]->op == GGML_OP_MUL_MAT) {
            for (int k = 0; k < 19 && i + 2 + k < cgraph->n_nodes; ++k) {
                char kb[64];
                snprintf(kb, sizeof(kb), "gdnwin_%02d_%s", k,
                         ggml_op_name(cgraph->nodes[i + 2 + k]->op));
                m1_op_time_add(std::string(kb), 1.0);
            }
        }
    }
    ...
    if (node->op == GGML_OP_MUL_MAT && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 gdn level-3 region for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    if (node->op == GGML_OP_GET_ROWS && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 gdn glue for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
    ...
    if (node->op == GGML_OP_L2_NORM && cuda_ctx->stream_context().concurrent_events.empty()) {
        ...
        if (nodes_to_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 gdn core for %s (skipped %d nodes)\n",
                          __func__, node->name, nodes_to_skip);
#endif
            return nodes_to_skip;
        }
        // batched decode (ns >= 2): the core fuse above bails, the pre-chain
        // fuses on its own and the delta-rule node keeps its own launch
        const int prep_skip = ggml_cuda_mach1_gdn_prep_fuse(*cuda_ctx, cgraph, i);
        if (prep_skip > 0) {
#ifdef GGML_CUDA_DEBUG
            GGML_LOG_INFO("%s: fused mach1 gdn prep chain for %s (skipped %d nodes)\n",
                          __func__, node->name, prep_skip);
#endif
            return prep_skip;
        }
    }
    ...
    if (cgraph->nodes[i]->op == GGML_OP_UNARY || cgraph->nodes[i]->op == GGML_OP_SOFT_MAX ||
        ...
        if (m1_op_time_mode() >= 2 && cgraph->nodes[i]->op == GGML_OP_SOFT_MAX) {
        ...
        if (can_fuse) {
            ...
            if (!args.delayed_softmax) {
                ...
                // measurement: the census exoneration was wrong at op granularity -
                // the precheck fires but the chain still executes, so name the
                // gate that rejects (same short-circuit order as the plain if)
                const bool f_sub = ggml_can_fuse_subgraph(cgraph, i, ops.size(), ops.data(), out_nodes, 2);
                const bool f_use = f_sub && ggml_cuda_should_use_topk_moe(node, logits, weights, ids);
                const bool f_mem = f_use && ggml_cuda_check_fusion_memory_ranges(cgraph, i, ops.size(), out_nodes, 2, /*is_topk_moe=*/true);
                if (f_mem) {
                    if (m1_op_time_mode() >= 2) {
                        m1_op_time_add("topk_moe:gate_ok", 1.0);
                    }
                    ggml_cuda_op_topk_moe(*cuda_ctx, logits, weights, ids, clamp, scale, bias, args);
                    return ops.size() - 1;
                }
                // GGML_MACH1_TOPK_FIX=1: the memrange rejection at nrows > 1 is the
                // fused outputs' pool ranges landing on LOGITS (the only non-elided
                // src - live tensors never overlap, so the hazard is exactly the
                // fused kernel clobbering a dead-but-still-being-read logits row of
                // another block). A pool-scratch copy of logits removes the hazard:
                // 2 launches instead of the 10-op chain. b16cen-s1: BOTH stacks
                // reject ~100 percent at B16 (~1.2 ms/step of chain ops) and
                // mach1's prefill rejects 22-47 percent of layers.
                static const bool topk_fix = [](){
                    const char * s = getenv("GGML_MACH1_TOPK_FIX");
                    return s != nullptr && atoi(s) != 0;
                }();
                if (f_use && topk_fix) {
                    if (m1_op_time_mode() >= 2) {
                        m1_op_time_add("topk_moe:gate_fixed", 1.0);
                    }
                    ggml_cuda_pool_alloc<char> lcopy(cuda_ctx->pool(), ggml_nbytes(logits));
                    CUDA_CHECK(cudaMemcpyAsync(lcopy.get(), logits->data, ggml_nbytes(logits),
                                               cudaMemcpyDeviceToDevice, cuda_ctx->stream()));
                    ggml_tensor logits_scratch = *logits;
                    logits_scratch.data = lcopy.get();
                    ggml_cuda_op_topk_moe(*cuda_ctx, &logits_scratch, weights, ids, clamp, scale, bias, args);
                    return ops.size() - 1;
                }
                if (m1_op_time_mode() >= 2) {
                    m1_op_time_add(std::string("topk_moe:gate_") +
                        (!f_sub ? "subgraph" : !f_use ? "should" : "memrange"), 1.0);
                }
...
static void ggml_cuda_graph_evaluate_and_capture(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, const bool use_cuda_graph, const bool cuda_graph_update_required, const void * graph_key) {
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                // GGML_MACH1_TIME >= 2: measurement-only per-OP timing keyed by
                // op name. Mode 2 wall-clocks with a stream sync around each
                // dispatch (~5-15 us of overhead per op - fine for counts,
                // distorting for avgs); mode 3 records CUDA event pairs in
                // stream order and drains them once per graph, giving true
                // stream occupancy per op. Both sync the stream, which is
                // illegal while capturing, so timing is skipped for captured
                // evals; set GGML_CUDA_DISABLE_GRAPHS=1 to time every eval.
                const bool m1_capturing = use_cuda_graph && cuda_graph_update_required;
                bool ok;
                if (!m1_capturing && (m1_op_time_mode() == 2 || m1_op_time_mode() == 3)) {
                    std::string key = ggml_op_name(node->op);
                    if (node->op == GGML_OP_UNARY) {
                        key += std::string(":") + ggml_unary_op_name(ggml_get_unary_op(node));
                    }
                    switch (node->op) {
                        case GGML_OP_MUL_MAT:
                        case GGML_OP_MUL_MAT_ID:
                        ...
                        case GGML_OP_MACH1_EXP_MM:
                        case GGML_OP_MACH1_NE_MM: {
                            // dst ne[2] disambiguates decode vs prefill instances of
                            // the same 2D print; src types split dispatch families
                            // (f32 vs f16 vs quantized weights take different lanes)
                            char b[128];
                            snprintf(b, sizeof(b), " s0=%ldx%ldx%ld d=%ldx%ldx%ld t0=%s%s%s",
                                     (long) node->src[0]->ne[0], (long) node->src[0]->ne[1],
                                     (long) node->src[0]->ne[2], (long) node->ne[0], (long) node->ne[1],
                                     (long) node->ne[2], ggml_type_name(node->src[0]->type),
                                     node->src[1] ? " t1=" : "",
                    ...
                    if (m1_op_time_mode() == 2) {
                        ...
                        CUDA_CHECK(cudaStreamSynchronize(cuda_ctx->stream()));
                        m1_op_time_add(key, std::chrono::duration<double, std::micro>(
                            std::chrono::steady_clock::now() - t0).count());
                    } else {
                        const int slot = m1_op_evt_begin(key, cuda_ctx->stream());
                        ok = ggml_cuda_compute_forward(*cuda_ctx, node);
                        m1_op_evt_end(slot, cuda_ctx->stream());
                    }
                } else {
                    ok = ggml_cuda_compute_forward(*cuda_ctx, node);
                }
            ...
            if (m1_op_time_mode() == 3 && !(use_cuda_graph && cuda_graph_update_required)) {
                m1_op_evt_drain(cuda_ctx->stream());
            }
        }
    ...
    if (use_cuda_graph) {
        ...
        // Launch graph
        if (m1_op_time_mode() >= 5 && !cuda_graph_update_required) {
            // measurement only: GPU window of the replay itself
            static cudaEvent_t ge0 = nullptr, ge1 = nullptr;
            if (ge0 == nullptr) {
                CUDA_CHECK(cudaEventCreate(&ge0));
                CUDA_CHECK(cudaEventCreate(&ge1));
            }
            CUDA_CHECK(cudaEventRecord(ge0, cuda_ctx->stream()));
            CUDA_CHECK(cudaGraphLaunch(graph->instance, cuda_ctx->stream()));
            CUDA_CHECK(cudaEventRecord(ge1, cuda_ctx->stream()));
            CUDA_CHECK(cudaEventSynchronize(ge1));
            float ms = 0.0f;
            CUDA_CHECK(cudaEventElapsedTime(&ms, ge0, ge1));
            m1_op_time_add("graphexec:replay_gpu", (double) ms*1000.0);
        } else {
            CUDA_CHECK(cudaGraphLaunch(graph->instance, cuda_ctx->stream()));
        }
#else
...
static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    const auto m1_t0 = std::chrono::steady_clock::now();
    ...
    if (m1_op_time_mode() >= 4) {
        // n_nodes separates the prefill and decode graph shapes, so the
        // counters attribute per phase
        char gk[64];
        snprintf(gk, sizeof(gk), "graphstat:%s:n%d",
                 !use_cuda_graph ? "direct"
                 : cuda_graph_update_required ? "capture" : "replay",
                 cgraph->n_nodes);
        m1_op_time_add(gk, 1.0);
    }
    ...
    if (m1_op_time_mode() >= 5) {
        m1_op_time_add("graphexec:eval_wall", std::chrono::duration<double, std::micro>(
            std::chrono::steady_clock::now() - m1_t0).count());
    }
...
static void ggml_backend_cuda_graph_optimize(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    // GGML_CUDA_CONCURRENT_ALL=1: the fork/join stream machinery below is
    // hard-gated to attn_norm with a fan-out of exactly 3 (the Q/K/V case it
    // was written for). The mach1 decode graph is mostly small latency-bound
    // ops with several other independent forks - the shared expert's gate/up
    // pair, the GDN qkv/gate pair - so let the same detection run on any fork
    // point when asked. Purely a scheduling change: each branch keeps its own
    // ops in their own order, so results are unaffected.
    // MEASURED (H200, paired, 3 rounds, token-identical): 90.21 with the
    // wider detection vs 86.70 with the attn_norm-only default, +4.0%.
    // The wider detection is untested outside mach1 graphs, so it defaults on
    // per graph: only when the graph contains mach1 ops; every other model
    // keeps upstream's attn_norm-only gate. An explicit
    // GGML_CUDA_CONCURRENT_ALL=0/1 overrides the default either way.
    static const int conc_all_env = [](){
        const char * s = getenv("GGML_CUDA_CONCURRENT_ALL");
        return s != nullptr ? (atoi(s) != 0 ? 1 : 0) : -1;  // -1 = unset
    }();
    bool graph_has_mach1 = false;
    for (int i = 0; i < cgraph->n_nodes && !graph_has_mach1; ++i) {
    ...
    const bool conc_all = conc_all_env >= 0 ? conc_all_env != 0 : graph_has_mach1;
    static const int conc_max = [](){
        const char * s = getenv("GGML_CUDA_CONCURRENT_MAX");
        return s ? atoi(s) : 4;
    }();
    const int min_fan_out = conc_all ? 2 : 3;
    const int max_fan_out = conc_all ? conc_max : 3;
    ...
    for (const auto & [root_node, count] : fan_out) {
        if (count >= min_fan_out && count <= max_fan_out) {
            ...
            if (!conc_all && !strstr(root_node->name, "attn_norm")) {
                continue;
            }
            ...
            // a node can consume the fork twice (ggml_mul(x, x)), so the
            // per-slot fan-out count can exceed the branch count; that region
            // is simply not a clean fork - skip it rather than abort.
            if (nodes_per_branch.size() != (size_t) count) {
                GGML_ASSERT(conc_all);
                continue;
            }
    ...
    switch (op->op) {
        ...
        case GGML_OP_SSM_CONV: {
            if (ggml_get_op_params_i32(op, 0) != 0) {
                // indexed single-token form: kernel bounds-checks channels, needs nc == 4
                return op->src[1]->ne[0] == 4;
            }
