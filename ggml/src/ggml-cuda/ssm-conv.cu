...
// indexed single-token form (ggml_ssm_conv_idx): per (seq, channel) read the
// d_conv-1 history floats from cache row state_idx[seq], the new column from
// x_new, emit the (optionally silu'd) conv value, and roll the window into
// cache row head+seq. Aliasing contract as the GDN indexed op: reads row
// state_idx[i], writes row head+i.
template <bool apply_silu, int NC>
static __global__ void ssm_conv_idx_f32(
        float * __restrict__ cache, const float * __restrict__ w,
        const int32_t * __restrict__ idx, const float * __restrict__ x_new,
        float * __restrict__ y,
        const int64_t cache_stride, const int64_t w_stride,
        const int64_t d_inner, const int head) {
    const int seq = blockIdx.y;
    const int ch  = blockIdx.x*blockDim.x + threadIdx.x;
    if (ch >= d_inner) {
        return;
    }
    const float * crow = cache + (int64_t) idx[seq]*cache_stride + (int64_t) ch*(NC - 1);
    float x[NC];
#pragma unroll
    for (int j = 0; j < NC - 1; ++j) {
        x[j] = crow[j];
    }
    x[NC - 1] = x_new[(int64_t) seq*d_inner + ch];
    float sumf = 0.0f;
#pragma unroll
    for (int j = 0; j < NC; ++j) {
        sumf += x[j]*w[(int64_t) ch*w_stride + j];
    }
    y[(int64_t) seq*d_inner + ch] = apply_silu ? ggml_cuda_op_silu_single(sumf) : sumf;
    float * drow = cache + (int64_t) (head + seq)*cache_stride + (int64_t) ch*(NC - 1);
#pragma unroll
    for (int j = 0; j < NC - 1; ++j) {
        drow[j] = x[j + 1];
    }
...
void ggml_cuda_op_ssm_conv(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * bias_add_node, ggml_tensor * silu_dst) {
    ...
    if (ggml_get_op_params_i32(dst, 0) != 0) {
        ...
        const ggml_tensor * src_idx = dst->src[2];
        const ggml_tensor * src_x   = dst->src[3];
        const int64_t nc      = src1->ne[0];
        const int64_t d_inner = src1->ne[1];
        const int64_t n_s     = src_idx->ne[0];
        const bool    silu    = ggml_get_op_params_i32(dst, 1) != 0;
        const int     head    = ggml_get_op_params_i32(dst, 2);
        GGML_ASSERT(nc == 4);
        const int threads = 256;
        const dim3 blocks((unsigned) ((d_inner + threads - 1)/threads), (unsigned) n_s, 1);
        const ggml_cuda_kernel_launch_params lp =
            ggml_cuda_kernel_launch_params(blocks, dim3(threads, 1, 1), 0, ctx.stream());
        if (silu) {
            ggml_cuda_kernel_launch(ssm_conv_idx_f32<true, 4>, lp,
                (float *) src0->data, (const float *) src1->data,
                (const int32_t *) src_idx->data, (const float *) src_x->data,
                (float *) dst->data,
                (int64_t) (src0->nb[1]/sizeof(float)), (int64_t) (src1->nb[1]/sizeof(float)),
                d_inner, head);
        } else {
            ggml_cuda_kernel_launch(ssm_conv_idx_f32<false, 4>, lp,
                (float *) src0->data, (const float *) src1->data,
                (const int32_t *) src_idx->data, (const float *) src_x->data,
                (float *) dst->data,
                (int64_t) (src0->nb[1]/sizeof(float)), (int64_t) (src1->nb[1]/sizeof(float)),
