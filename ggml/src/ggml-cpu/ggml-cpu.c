...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_ARGMAX_MASKED:
            {
                ggml_compute_forward_argmax_masked(params, tensor);
            } break;
        ...
        case GGML_OP_MACH1_NE_MM:
            {
                ggml_compute_forward_mach1_ne_mm(params, tensor);
            } break;
        case GGML_OP_MACH1_EMBED_ROWS:
            {
                ggml_compute_forward_mach1_embed_rows(params, tensor);
            } break;
        case GGML_OP_MACH1_EXP_MM:
            {
                ggml_compute_forward_mach1_exp_mm(params, tensor);
            } break;
        case GGML_OP_MACH1_EXP_BASIS:
            {
                ggml_compute_forward_mach1_exp_basis(params, tensor);
            } break;
        case GGML_OP_MACH1_RT_MM:
            {
                ggml_compute_forward_mach1_rt_mm(params, tensor);
            } break;
        case GGML_OP_MACH1_HEAD_MM:
            {
                ggml_compute_forward_mach1_head_mm(params, tensor);
            } break;
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!ggml_cpu_extra_work_size(n_threads, node, &cur)) {
            switch (node->op) {
                ...
                case GGML_OP_MACH1_RT_MM:
                    {
                        // u[nt][n] + v[nt][m]: the walk decodes into the
                        // accumulator, so no m*n weight buffer is needed
                        const int64_t n  = node->src[1]->ne[0];                        // su
                        const int64_t m  = node->src[2]->ne[0];                        // sv
                        const struct ggml_tensor * x = node->src[4];
                        const int64_t nt = x->ne[1]*x->ne[2]*x->ne[3];
                        // + a token-major copy of u for the batched walk, which
                        // contracts with tokens in the vector lanes (see
