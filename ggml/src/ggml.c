...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "mach1_ne_mm(packed, gscale, lut, x)",
    "mach1_embed_rows(q, mn, mx, ids)",
    "mach1_exp_mm(kept, dem, su, sv, tlut, remap, ids, x[, gamma])",
    "mach1_exp_basis(a, b, c, remap, ids, x[, acc])",
    "mach1_rt_mm(trellis, su, sv, tlut, x)",
    "mach1_head_mm(qp, gscale, x)",
    "mach1_embed_gather(codes, lut, ids)",
...
struct ggml_tensor * ggml_argmax_masked(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        const int32_t       * masked,
        size_t                n_masked) {
    GGML_ASSERT(ggml_is_matrix(a));
    GGML_ASSERT(a->ne[0] <= INT32_MAX);
    GGML_ASSERT(n_masked <= GGML_MAX_OP_PARAMS / sizeof(int32_t) - 1);
    GGML_ASSERT(n_masked == 0 || masked != NULL);

    struct ggml_tensor * result = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, a->ne[1]);

    result->op     = GGML_OP_ARGMAX_MASKED;
    result->src[0] = a;

    int32_t params[GGML_MAX_OP_PARAMS / sizeof(int32_t)] = { 0 };
    params[0] = (int32_t) n_masked;
    for (size_t i = 0; i < n_masked; ++i) {
        GGML_ASSERT(masked[i] >= 0 && masked[i] < a->ne[0]);
        params[i + 1] = masked[i];
    }
    ggml_set_op_params(result, params, sizeof(params));
...
// single-token indexed conv: the kernel reads each sequence's (d_conv-1)
// history floats per channel from conv cache row state_idx[seq], takes the
// new column from x_new, applies the conv (and silu when asked), and rolls
// the window back into cache row head+seq. Replaces the gather, concat,
// conv, silu, and state-write nodes at decode.
struct ggml_tensor * ggml_ssm_conv_idx(
        struct ggml_context * ctx,
        struct ggml_tensor  * cache_rows,
        struct ggml_tensor  * c,
    ...
    GGML_ASSERT(cache_rows->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous(cache_rows));
    GGML_ASSERT(ggml_is_contiguous(x_new));

    const int64_t d_conv  = c->ne[0];
    const int64_t d_inner = c->ne[1];
    const int64_t n_s     = state_idx->ne[0];

    GGML_ASSERT(cache_rows->ne[0] == (d_conv - 1)*d_inner);
    GGML_ASSERT(ggml_nelements(x_new) == d_inner*n_s);
    GGML_ASSERT(head >= 0 && head + n_s <= cache_rows->ne[1]);

    struct ggml_tensor * result = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, d_inner, 1, n_s);

    ggml_set_op_params_i32(result, 0, 1);   // indexed single-token form
    ggml_set_op_params_i32(result, 1, silu ? 1 : 0);
    ggml_set_op_params_i32(result, 2, head);

    result->op     = GGML_OP_SSM_CONV;
    result->src[0] = cache_rows;
    result->src[1] = c;
    result->src[2] = state_idx;
    result->src[3] = x_new;
...
// state_rows is the raw recurrent cache [row_size, mem_size]; state_idx is I32
// [n_seqs] source-row indices (build_rs's s_copy head). The kernel gathers rows
// at load, replacing the materialized get_rows copy of the state.
struct ggml_tensor * ggml_gated_delta_net_idx(
    ...
    GGML_ASSERT(state_idx->type  == GGML_TYPE_I32);
    GGML_ASSERT(ggml_is_contiguous(state_rows));
    GGML_ASSERT(state_rows->ne[0] == S_v * S_v * H);
    GGML_ASSERT(state_idx->ne[0]  == n_seqs);

    // shape the dummy state view the shared constructor asserts against
    struct ggml_tensor * state_view = ggml_reshape_4d(ctx,
        ggml_view_2d(ctx, state_rows, state_rows->ne[0], n_seqs, state_rows->nb[1], 0),
        S_v, S_v, H, n_seqs);
    struct ggml_tensor * result = ggml_gated_delta_net(ctx, q, k, v, g, beta, state_view, K);

    ggml_set_op_params_i32(result, 1, 1);   // indexed-state form
...
struct ggml_tensor * ggml_mach1_ne_mm(
        struct ggml_context * ctx,
...
// ggml_mach1_embed_rows

struct ggml_tensor * ggml_mach1_embed_rows(
        struct ggml_context * ctx,
        struct ggml_tensor  * q,
    ...
    GGML_ASSERT((n_embd/mn->ne[0])*3 % 8 == 0);   // GPU kernels assume byte-aligned group bitstreams

    const int64_t ne[4] = { n_embd, ids->ne[0], 1, 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op     = GGML_OP_MACH1_EMBED_ROWS;
    result->src[0] = q;
    result->src[1] = mn;
    result->src[2] = mx;
    result->src[3] = ids;
...
struct ggml_tensor * ggml_mach1_exp_mm(
        struct ggml_context * ctx,
    ...
    GGML_ASSERT(m <= 4096);   // CPU kernel column scratch bound
    ...
    result->op     = GGML_OP_MACH1_EXP_MM;
...
struct ggml_tensor * ggml_mach1_rt_mm(
    ...
    GGML_ASSERT(tlut->type == GGML_TYPE_F16);   // pre-rounded (see ggml_mach1_exp_mm)
    GGML_ASSERT(x->type    == GGML_TYPE_F32);
...
struct ggml_tensor * ggml_mach1_head_mm(
        struct ggml_context * ctx,
    ...
    GGML_ASSERT(n % 64 == 0);
    GGML_ASSERT(n <= 8192);   // CPU kernel row scratch bound
    ...
    result->op     = GGML_OP_MACH1_HEAD_MM;
    result->src[0] = qp;
    result->src[1] = gscale;
    result->src[2] = x;
...
// ggml_mach1_embed_gather

struct ggml_tensor * ggml_mach1_embed_gather(
        struct ggml_context * ctx,
    ...
    GGML_ASSERT(lut->ne[0] == 16 && lut->ne[1] == vocab*(n_embd/64));

    const int64_t ne[4] = { n_embd, ids->ne[0], 1, 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op     = GGML_OP_MACH1_EMBED_GATHER;
    result->src[0] = codes;
    result->src[1] = lut;
    result->src[2] = ids;
...
// ggml_mach1_exp_basis

struct ggml_tensor * ggml_mach1_exp_basis(
        struct ggml_context * ctx,
    ...
    result->op     = GGML_OP_MACH1_EXP_BASIS;
