...
extern "C" {
    ...
    enum ggml_op {
        ...
        GGML_OP_MACH1_NE_MM,
        GGML_OP_MACH1_EMBED_ROWS,
        GGML_OP_MACH1_EXP_MM,
        GGML_OP_MACH1_EXP_BASIS,
        GGML_OP_MACH1_RT_MM,
        GGML_OP_MACH1_HEAD_MM,
        GGML_OP_MACH1_EMBED_GATHER,
    ...
    // argmax along rows while excluding up to 15 fixed column indices
    GGML_API struct ggml_tensor * ggml_argmax_masked(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
    ...
    // single-token indexed conv over recurrent cache rows (see ggml.c)
    ...
    */
    ...
    // indexed-state form: state_rows is the raw recurrent cache
    // [S_v*S_v*H, mem_size] and state_idx (I32 [n_seqs]) names each
    // sequence's source row; the kernel gathers at load instead of
    // consuming a materialized get_rows copy
    ...
    // Mach-1 transform-free Lloyd bitshift-trellis GEMM (L=12, group-128).
    ...
    GGML_API struct ggml_tensor * ggml_mach1_ne_mm(
            struct ggml_context * ctx,
    ...
    // Mach-1 int3 asymmetric grouped embedding row gather.
    ...
    GGML_API struct ggml_tensor * ggml_mach1_embed_rows(
            struct ggml_context * ctx,
            struct ggml_tensor  * q,
    ...
    // Mach-1 trellis routed-expert matmul (expert tier).
    ...
    //   su: F16 [n, n_expert]   sv: F16 [m, n_expert]   tlut: F32 or F16 [V, 2^tlut_bits]
    ...
    GGML_API struct ggml_tensor * ggml_mach1_exp_mm(
            struct ggml_context * ctx,
    ...
    // Mach-1 shared low-rank basis residual for demoted experts:
    // y = B (c_e ⊙ (A x)) in fp32 (v = A x sequential dots; t = c*v one
    // rounding; y_i accumulates fl(B_ij * t_j) sequentially — kept slots
    // produce zeros). Same remap/ids convention as ggml_mach1_exp_mm.
    ...
    //   x: F32 [n, 1 | n_used, n_tokens]  ->  dst F32 [m, n_used, n_tokens]
    GGML_API struct ggml_tensor * ggml_mach1_exp_basis(
            struct ggml_context * ctx,
    ...
    GGML_API struct ggml_tensor * ggml_mach1_rt_mm(
            struct ggml_context * ctx,
    ...
    // Mach-1 int5-g64 head matmul (payload v3 lm_head; codec int5g64_packed).
    ...
    GGML_API struct ggml_tensor * ggml_mach1_head_mm(
            struct ggml_context * ctx,
            struct ggml_tensor  * qp,
            struct ggml_tensor  * gscale,
            struct ggml_tensor  * x);

    // Mach-1 lossless nibble-LUT embedding gather (payload v3 embed).
    ...
    //   ids: I32 [n_tokens]  ->  dst F32 [n_embd, n_tokens]
    GGML_API struct ggml_tensor * ggml_mach1_embed_gather(
            struct ggml_context * ctx,
