...
struct llm_build_delta_net_base : public llm_graph_context {
    ...
    // indexed-state fast path (set per layer before build_recurrent_attn):
    // the fused AR op gathers state rows through these instead of consuming
    // the materialized build_rs get_rows copy
    ggml_tensor * gdn_state_rows = nullptr;
    ggml_tensor * gdn_state_idx  = nullptr;
...
// quantized matrix with mach1 packed sidecars, decoded in-kernel by the
// GGML_OP_MACH1_* ops the graph routes every dense projection through.
struct llama_model_mach1 : public llama_model_qwen35moe {
    llama_model_mach1(const struct llama_model_params & params) : llama_model_qwen35moe(params) {}
    void load_arch_tensors(llama_model_loader & ml) override;
    std::unique_ptr<llm_graph_context> build_arch_graph(const llm_graph_params & params) const override;
    ...
    struct graph : public llm_build_delta_net_base {
        graph(const llama_model_mach1 & model, const llm_graph_params & params);
    private:
