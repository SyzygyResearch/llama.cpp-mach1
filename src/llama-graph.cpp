...
void llm_graph_context::build_sampling() const {
    ...
    // The common temperature=0 serving path is a single immutable operation:
    // exclude the (usually five) EOG token ids and take argmax. When every
    // active sequence has that same descriptor, process the full logits matrix
    // in one op instead of instantiating a sampler graph per slot.
    std::vector<llama_token> common_greedy_mask;
    bool batched_greedy = !seq_to_logit_row.empty() &&
            res->t_logits->ne[1] == (int64_t) seq_to_logit_row.size();
    bool first_greedy = true;
    if (batched_greedy) {
        for (const auto & [seq_id, row_idx] : seq_to_logit_row) {
            GGML_UNUSED(row_idx);

            const auto sampler_it = samplers.find(seq_id);
            if (sampler_it == samplers.end()) {
                batched_greedy = false;
                break;
            }

            std::vector<llama_token> mask;
            if (!llama_sampler_chain_get_greedy_mask(sampler_it->second, mask)) {
                batched_greedy = false;
                break;
            }
            if (first_greedy) {
                common_greedy_mask = std::move(mask);
                first_greedy = false;
            } else if (mask != common_greedy_mask) {
                batched_greedy = false;
                break;
            }
        }
    ...
    if (batched_greedy && !first_greedy) {
        ggml_tensor * sampled_all = ggml_argmax_masked(
                ctx0,
                res->t_logits,
                common_greedy_mask.data(),
                common_greedy_mask.size());
        ggml_set_name(sampled_all, "greedy_masked_argmax_batched");

        for (const auto & [seq_id, row_idx] : seq_to_logit_row) {
            ggml_tensor * sampled = ggml_view_1d(
                    ctx0,
                    sampled_all,
                    1,
                    row_idx*sizeof(llama_token));
            ggml_format_name(sampled, "greedy_masked_token_%d", seq_id);
            res->t_sampled[seq_id] = sampled;
            ggml_build_forward_expand(gf, sampled);
        }

        static std::once_flag once;
        std::call_once(once, [&]() {
            LLAMA_LOG_INFO("%s: using batched masked greedy sampling (rows=%" PRId64 ", masked=%zu)\n",
                    __func__, res->t_logits->ne[1], common_greedy_mask.size());
        });
        return;
    }
