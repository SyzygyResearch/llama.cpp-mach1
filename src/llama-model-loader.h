...
struct llama_model_loader {
    ...
    // mach1 codec checkpoints: the weight ENCODING is ours, the topology is
    // the stock base architecture. Aligned files therefore declare
    // general.architecture = "<base>" (so hparams, rope type, recurrent and
    // rollback support all resolve through the base arch, and any gguf tool
    // reads the file as what it is) and mark the encoding with
    // "mach1.format_version". Legacy files declaring general.architecture =
    // "mach1" keep working unchanged - see llama_model_create.
    bool has_mach1_codec = false;
