...
static void llama_model_quantize_impl(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    // arch OR codec marker: an aligned checkpoint declares its base
    // architecture, so the arch alone no longer identifies a codec file
    if (model->arch == LLM_ARCH_MACH1 || ml.has_mach1_codec) {
        throw std::runtime_error(
            "mach1 checkpoints are already packed trellis code streams; "
