...
// greedy with a small immutable token denylist

struct llama_sampler_greedy_masked : public llama_sampler_backend {
    const std::vector<llama_token> masked_tokens;
};

static const char * llama_sampler_greedy_masked_name(const struct llama_sampler * smpl) {
    auto * sctx = (llama_sampler_greedy_masked *) smpl->ctx;
    return sctx->get_name();
}

static void llama_sampler_greedy_masked_apply(
        struct llama_sampler * smpl,
        llama_token_data_array * cur_p) {
    const auto * sctx = (const llama_sampler_greedy_masked *) smpl->ctx;

    cur_p->selected = -1;
    for (size_t i = 0; i < cur_p->size; ++i) {
        const llama_token id = cur_p->data[i].id;
        if (std::find(sctx->masked_tokens.begin(), sctx->masked_tokens.end(), id) != sctx->masked_tokens.end()) {
            continue;
        }
        if (cur_p->selected < 0 || cur_p->data[i].logit > cur_p->data[cur_p->selected].logit) {
            cur_p->selected = i;
        }
    }

    // all candidates masked - fall back to the first so the selection stays valid
    if (cur_p->selected < 0 && cur_p->size > 0) {
        cur_p->selected = 0;
    }
}

static struct llama_sampler * llama_sampler_greedy_masked_clone(const struct llama_sampler * smpl) {
    const auto * sctx = (const llama_sampler_greedy_masked *) smpl->ctx;
    return llama_sampler_init_greedy_masked(sctx->masked_tokens);
}

static void llama_sampler_greedy_masked_free(struct llama_sampler * smpl) {
    delete (llama_sampler_greedy_masked *) smpl->ctx;
}

static bool llama_sampler_greedy_masked_backend_init(
        struct llama_sampler * smpl,
        ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_greedy_masked *) smpl->ctx;
    const bool res = llama_sampler_backend_support(smpl, buft);
...
static void llama_sampler_greedy_masked_backend_apply(
        struct llama_sampler * smpl,
        struct ggml_context * ctx,
        struct ggml_cgraph * gf,
        struct llama_sampler_data * data) {
    ...
    data->sampled = ggml_argmax_masked(
            ctx,
            data->logits,
            sctx->masked_tokens.data(),
            sctx->masked_tokens.size());
    ggml_set_name(data->sampled, "greedy_masked_argmax");
}

static struct llama_sampler_i llama_sampler_greedy_masked_i = {
    /* .name              = */ llama_sampler_greedy_masked_name,
    /* .accept            = */ nullptr,
    /* .apply             = */ llama_sampler_greedy_masked_apply,
    /* .reset             = */ nullptr,
    /* .clone             = */ llama_sampler_greedy_masked_clone,
    /* .free              = */ llama_sampler_greedy_masked_free,
    /* .backend_init      = */ llama_sampler_greedy_masked_backend_init,
    /* .backend_accept    = */ nullptr,
    /* .backend_apply     = */ llama_sampler_greedy_masked_backend_apply,
    /* .backend_set_input = */ nullptr,
};

struct llama_sampler * llama_sampler_init_greedy_masked(
        const std::vector<llama_token> & masked_tokens) {
    GGML_ASSERT(masked_tokens.size() <= GGML_MAX_OP_PARAMS / sizeof(int32_t) - 1);
    return llama_sampler_init(
        /* .iface = */ &llama_sampler_greedy_masked_i,
        /* .ctx   = */ new llama_sampler_greedy_masked {
            ("greedy-masked"),
...
bool llama_sampler_chain_get_greedy_mask(
        const struct llama_sampler * chain,
        std::vector<llama_token> & masked_tokens) {
    if (!chain || chain->iface != &llama_sampler_chain_i) {
        return false;
    }

    const auto * cctx = (const llama_sampler_chain *) chain->ctx;
    if (cctx->samplers.size() != 1 || cctx->samplers[0].ptr->iface != &llama_sampler_greedy_masked_i) {
        return false;
    }

    const auto * sctx = (const llama_sampler_greedy_masked *) cctx->samplers[0].ptr->ctx;
    masked_tokens = sctx->masked_tokens;
    return true;
}
