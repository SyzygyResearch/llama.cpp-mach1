...
llama_model_loader::llama_model_loader(
    ...
    if (!fname.empty()) {
        ...
        // the codec marker is an arch-independent literal key, so it is read
        // the same way whichever architecture the file declares
        {
            uint32_t m1_ver = 0;
            has_mach1_codec = arch_name == "mach1" ||
                              get_key("mach1.format_version", m1_ver, false);
        }
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        if (!buft) {
            // mach1 packed sidecar tensors ("<base>.m1_*") are raw code streams
            ...
            // extra (repack) bufts whose supports_op veto rejects the mach1 ops.
