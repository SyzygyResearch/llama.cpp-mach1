...
static void test_backend_batched_masked_greedy_sampling(const test_params & params) {
    constexpr int32_t n_seqs = 16;

    // Pin the reference semantics independently of model logits: a masked
    // winner is skipped and equal surviving logits choose the lower token id.
    {
        llama_sampler_ptr sampler(llama_sampler_init_greedy_masked({ 2 }));
    ...
    std::vector<llama_token> masked;
    for (llama_token token = 0; token < n_vocab; ++token) {
        if (llama_vocab_is_eog(vocab, token)) {
            masked.push_back(token);
            if (masked.size() == GGML_MAX_OP_PARAMS / sizeof(int32_t) - 1) {
                break;
            }
        }
    }
    ...
    for (llama_seq_id seq_id = 0; seq_id < n_seqs; ++seq_id) {
        prompts.emplace(seq_id, "Batched masked greedy parity " + std::to_string(seq_id));
    }

    std::vector<llama_sampler_seq_config> no_configs;
    test_context reference(params, no_configs, n_seqs);
    GGML_ASSERT(reference.decode(prompts));

    std::vector<llama_token> expected(n_seqs, LLAMA_TOKEN_NULL);
    for (llama_seq_id seq_id = 0; seq_id < n_seqs; ++seq_id) {
        const float * logits = llama_get_logits_ith(reference.ctx.get(), reference.idx_for_seq(seq_id));
        GGML_ASSERT(logits != nullptr);

        float best_value = -INFINITY;
        for (llama_token token = 0; token < n_vocab; ++token) {
            if (std::find(masked.begin(), masked.end(), token) != masked.end()) {
                continue;
            }
            if (logits[token] > best_value) {
                best_value = logits[token];
                expected[seq_id] = token;
            }
        }
        GGML_ASSERT(expected[seq_id] != LLAMA_TOKEN_NULL);
    }

    std::vector<common_sampler_ptr> samplers;
    std::vector<llama_sampler_seq_config> configs;
    samplers.reserve(n_seqs);
    configs.reserve(n_seqs);
    for (llama_seq_id seq_id = 0; seq_id < n_seqs; ++seq_id) {
        common_params_sampling sparams;
        sparams.backend_sampling = true;
        sparams.temp = 0.0f;
        sparams.n_probs = 0;
        for (const llama_token token : masked) {
            sparams.logit_bias.push_back({ token, -INFINITY });
        }

        samplers.emplace_back(common_sampler_init(model, sparams));
        configs.push_back({ seq_id, common_sampler_get(samplers.back().get()) });
    }
    GGML_ASSERT(common_sampler_print(samplers.front().get()).find("greedy-masked") != std::string::npos);

    // A finite bias is not a denylist and must retain the stock sampler chain.
    {
        common_params_sampling fallback_params;
        fallback_params.backend_sampling = true;
        fallback_params.temp = 0.0f;
        fallback_params.logit_bias.push_back({ 0, -1.0f });
        common_sampler_ptr fallback(common_sampler_init(model, fallback_params));
        GGML_ASSERT(common_sampler_print(fallback.get()).find("greedy-masked") == std::string::npos);
    }

    test_context fast(params, configs, n_seqs);
    GGML_ASSERT(fast.decode(prompts));

    for (llama_seq_id seq_id = 0; seq_id < n_seqs; ++seq_id) {
        const int32_t idx = fast.idx_for_seq(seq_id);
        const llama_token backend_token = llama_get_sampled_token_ith(fast.ctx.get(), idx);
        GGML_ASSERT(backend_token == expected[seq_id]);

        // Also exercise common_sampler_sample(): the ID-only fast path must not
        // try to read or materialize a full host logits row.
        const llama_token common_token = common_sampler_sample(samplers[seq_id].get(), fast.ctx.get(), idx);
        GGML_ASSERT(common_token == expected[seq_id]);
    }

    printf("backend batched masked greedy sampling test PASSED (%d rows, %zu masks)\n", n_seqs, masked.size());
}
...
static const backend_test_case BACKEND_TESTS[] = {
    ...
    { "greedy_masked_batched", test_backend_batched_masked_greedy_sampling, true },
