...
static void test(void) {
    ...
    argv = {"binary_name", "--batch-wait-us", "-1"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SERVER));

    argv = {"binary_name", "--batch-wait-us", "100001"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SERVER));
    ...
    argv = {"binary_name", "--batch-wait-us", "1000"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SERVER));
    assert(params.batch_wait_us == 1000);

    argv = {"binary_name", "--batch-wait-us", "100000"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SERVER));
    assert(params.batch_wait_us == 100000);
