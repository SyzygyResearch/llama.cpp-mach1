...
// GGML_OP_MACH1_* codec ops. Any random bitstream is a valid trellis code, so
...
struct test_mach1_ne_mm : public test_case {
    ...
    test_mach1_ne_mm(int64_t B = 64, int64_t T = 512, int64_t k = 4, int64_t n_chunks = 1, int64_t n_tokens = 3)
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        return ggml_mach1_ne_mm(ctx, packed, gscale, lut, x);
    }
...
struct test_mach1_embed_rows : public test_case {
    ...
    test_mach1_embed_rows(int64_t n_vocab = 128, int64_t n_embd = 256, int64_t ng = 4, int64_t n_tokens = 5)
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        return ggml_mach1_embed_rows(ctx, q, mn, mx, ids);
    }
...
struct test_mach1_exp_common : public test_case {
    const int64_t m, n, n_kept, n_dem, n_used, n_tok;
    const bool per_slot_x;

    test_mach1_exp_common(bool per_slot_x,
        ...
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            if (t->type == GGML_TYPE_I16 || t->type == GGML_TYPE_I8) {
                ...
                for (int64_t i = 0; i < n_dem; i++) {
                    data.push_back((int32_t) i | MACH1_TEST_DEM_FLAG);
                }
...
struct test_mach1_exp_mm : public test_mach1_exp_common {
    const bool v8;   // payload v3: V=8/K=1.5 walk, tlut [8,32768], wave_gamma, no dem

    test_mach1_exp_mm(bool per_slot_x = false, bool v8 = false,
            int64_t m = 64, int64_t n = 128, int64_t n_kept = 3, int64_t n_dem = 2,
            int64_t n_used = 2, int64_t n_tok = 3)
        : test_mach1_exp_common(per_slot_x, m, n, n_kept, n_dem, n_used, n_tok), v8(v8) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        // v3 ships the tlut pre-rounded to fp16 (bit-exact; see ggml_mach1_exp_mm)
        ...
        return ggml_mach1_exp_mm(ctx, kept, dem, su, sv, tlut, remap, ids, x, gamma);
    }

    void initialize_tensors(ggml_context * ctx) override {
        if (!v8) {
            test_mach1_exp_common::initialize_tensors(ctx);
            return;
        }
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
...
struct test_mach1_rt_mm : public test_case {
    ...
    test_mach1_rt_mm(int64_t m = 64, int64_t n = 128, int64_t n_tok = 3)
        : m(m), n(n), n_tok(n_tok) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        return ggml_mach1_rt_mm(ctx, trellis, su, sv, tlut, x);
    }
...
struct test_mach1_head_mm : public test_case {
    const int64_t n, vocab, n_tok;

    test_mach1_head_mm(int64_t n = 128, int64_t vocab = 96, int64_t n_tok = 3)
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        ggml_tensor * x  = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, n_tok);
        return ggml_mach1_head_mm(ctx, qp, gs, x);
    }
...
struct test_mach1_embed_gather : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        return ggml_mach1_embed_gather(ctx, codes, lut, ids);
    }
...
struct test_mach1_exp_basis : public test_mach1_exp_common {
    const int64_t r = 16;
    const bool with_acc;

    test_mach1_exp_basis(bool per_slot_x = false, bool with_acc = false)
        : test_mach1_exp_common(per_slot_x), with_acc(with_acc) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        return ggml_mach1_exp_basis(ctx, a, b, c, remap, ids, x, acc);
    }
};
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_mach1_ne_mm(64, 512, 4, 1, 1));
    test_cases.emplace_back(new test_mach1_ne_mm(64, 512, 3, 2, 5));
    test_cases.emplace_back(new test_mach1_embed_rows());
    test_cases.emplace_back(new test_mach1_exp_mm(false));
    test_cases.emplace_back(new test_mach1_exp_mm(true));
    test_cases.emplace_back(new test_mach1_exp_mm(false, true));
    test_cases.emplace_back(new test_mach1_exp_mm(true,  true));
    // few experts + many pairs: forces a group past MACH1_EXP_FUSE_MAX_PAIRS so
    // the CPU materializing path is exercised too
    test_cases.emplace_back(new test_mach1_exp_mm(false, false, 64, 128, 1, 1, 4, 8));
    ...
    test_cases.emplace_back(new test_mach1_exp_basis(true));
    test_cases.emplace_back(new test_mach1_exp_basis(false, true));
    test_cases.emplace_back(new test_mach1_exp_basis(true,  true));
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    // MACH1 real Mach-1-Small-Additive shapes (n_embd 2048, 256 experts v8,
    // n_ff_exp 512, n_used 8; ubatch 512 = prefill, n_tok 1 = decode)
    for (int64_t n_tok : {(int64_t) 512, (int64_t) 1}) {
        test_cases.emplace_back(new test_mach1_exp_mm(false, true,  512, 2048, 256, 0, 8, n_tok)); // gate/up
        test_cases.emplace_back(new test_mach1_exp_mm(false, true, 2048,  512, 256, 0, 8, n_tok)); // down
        test_cases.emplace_back(new test_mach1_rt_mm(8192, 2048, n_tok));   // qkvz proj
