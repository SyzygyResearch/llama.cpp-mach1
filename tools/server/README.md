...
| `--batch-wait-us N` | fixed wait in microseconds to collect synchronized inference requests, trades decode loop latency for larger batches, range 0 to 100000 (default: 0, 0 = disabled)<br/>(env: LLAMA_ARG_BATCH_WAIT_US) |
...
Some toggles are available only as environment variables, without a command line argument:

- `LLAMA_SERVER_BATCHED_RESPONSES`: when set to a non-zero value, the responses produced by one decode step are published together at the end of the step (one queue lock and one reader notification) instead of one by one. Default: disabled. Primarily useful for high-concurrency serving.
