...
void server_queue::start_loop(int64_t idle_sleep_ms, int64_t batch_wait_us) {
    running = true;
    time_last_task = ggml_time_ms();
    bool batch_wait_logged = false;
    ...
    auto wait_for_inference_batch = [&](std::unique_lock<std::mutex> & lock) {
        // caller must hold mutex_tasks
        if (batch_wait_us <= 0 || !running) {
            return;
        }
        const auto is_inference_task = [](const server_task & task) {
            return task.need_embd() || task.need_logits();
        };
        const size_t inference_tasks_before =
            std::count_if(queue_tasks.begin(), queue_tasks.end(), is_inference_task);
        if (inference_tasks_before > 0) {
            const auto wait_started = std::chrono::steady_clock::now();
            condition_tasks.wait_for(lock, std::chrono::microseconds(batch_wait_us), [&] {
                return !running;
            });
            const auto wait_elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - wait_started).count();
            if (running && !batch_wait_logged) {
                const size_t inference_tasks_after =
                    std::count_if(queue_tasks.begin(), queue_tasks.end(), is_inference_task);
                QUE_INF("batch_wait_us: waited to batch concurrent requests, inference_before = %zu, inference_after = %zu, actual_wait_us = %lld\n",
                        inference_tasks_before, inference_tasks_after, (long long) wait_elapsed_us);
                batch_wait_logged = true;
            }
        }
    };
...
bool server_response::enqueue_waiting_result(server_task_result_ptr && result) {
...
void server_response::send(server_task_result_ptr && result) {
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_results);
        queued = enqueue_waiting_result(std::move(result));
    }
...
void server_response::send(std::vector<server_task_result_ptr> && results) {
    size_t n_queued = 0;
    {
        std::unique_lock<std::mutex> lock(mutex_results);
        for (auto & result : results) {
            if (enqueue_waiting_result(std::move(result))) {
                ++n_queued;
            }
        }
    }
    ...
    if (n_queued > 0) {
        if (n_queued > 1) {
            ...
            std::call_once(once, [n_queued]() {
                RES_INF("batched responses: publishing %zu results in one notification\n", n_queued);
            });
        }
        condition_results.notify_all();
    }
...
server_response_batch::server_response_batch(server_response & responses) : responses(responses) {}

server_response_batch::~server_response_batch() {
    flush();
}

void server_response_batch::send(server_task_result_ptr && result) {
    pending.emplace_back(std::move(result));
}

void server_response_batch::flush() {
    if (pending.empty()) {
        return;
    }

    auto results = std::move(pending);
    pending.clear();
    responses.send(std::move(results));
}
