...
public:
     ...
     * - Exit sleeping state
     *
     * If batch_wait_us > 0, inference-task wake-ups wait once before processing so
     * requests arriving in the same window can be handled by one slot update.
     */
    void start_loop(int64_t idle_sleep_ms = -1, int64_t batch_wait_us = 0);
...
private:
    ...
    // Caller must hold mutex_results. Returns true when the result was queued.
    bool enqueue_waiting_result(server_task_result_ptr && result);
...
public:
    ...
    // Send multiple results with one queue lock and one reader notification.
    // Results retain their input order, including multiple results for one task.
    void send(std::vector<server_task_result_ptr> && results);
...
// Collects responses produced by one logical server step and publishes them
// together. Destruction flushes pending responses so errors cannot overtake
// already-produced partial results while unwinding.
struct server_response_batch {
    explicit server_response_batch(server_response & responses);
    ~server_response_batch();

    server_response_batch(const server_response_batch &) = delete;
    server_response_batch & operator=(const server_response_batch &) = delete;
