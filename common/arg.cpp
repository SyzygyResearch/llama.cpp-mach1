...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--batch-wait-us"}, "N",
        string_format("fixed wait in microseconds to collect synchronized inference requests, trades decode loop latency for larger batches, range 0 to 100000 (default: %d, 0 = disabled)", params.batch_wait_us),
        [](common_params & params, int value) {
            if (value < 0 || value > 100000) {
                throw std::invalid_argument("error: batch wait must be between 0 and 100000\n");
            }
            params.batch_wait_us = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_BATCH_WAIT_US"));
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
