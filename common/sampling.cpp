...
static bool common_sampler_backend_greedy_mask(
        const struct common_params_sampling & params,
        int32_t n_vocab,
        std::vector<llama_token> & masked_tokens) {
    if (!std::isfinite(params.temp) || !std::isfinite(params.dynatemp_range) ||
        !std::isfinite(params.dynatemp_exponent) || !std::isfinite(params.top_p) ||
        !std::isfinite(params.min_p) || !std::isfinite(params.top_n_sigma) ||
        !std::isfinite(params.typ_p) || !std::isfinite(params.xtc_probability) ||
        !std::isfinite(params.xtc_threshold)) {
        return false;
    }

    if (!params.backend_sampling || params.n_probs != 0 || params.mirostat != 0 ||
    ...
    for (const auto sampler : params.samplers) {
        switch (sampler) {
            ...
            case COMMON_SAMPLER_TYPE_MIN_P:
                // These filters cannot remove the global maximum.
                break;
    ...
    for (const auto & bias : params.logit_bias) {
        if (bias.token < 0 || bias.token >= n_vocab || !std::isinf(bias.bias) || bias.bias >= 0.0f) {
            return false;
        }
        if (std::find(masked_tokens.begin(), masked_tokens.end(), bias.token) == masked_tokens.end()) {
            masked_tokens.push_back(bias.token);
        }
    }
    std::sort(masked_tokens.begin(), masked_tokens.end());

    return masked_tokens.size() <= GGML_MAX_OP_PARAMS / sizeof(int32_t) - 1;
}
...
struct common_sampler * common_sampler_init(const struct llama_model * model, struct common_params_sampling & params) {
    ...
    std::vector<llama_token> greedy_mask;
    const bool backend_greedy_fast = !grmr && !rbudget && common_sampler_backend_greedy_mask(
            params, llama_vocab_n_tokens(vocab), greedy_mask);

    if (!backend_greedy_fast && params.has_logit_bias()) {
        samplers.push_back(llama_sampler_init_logit_bias(llama_vocab_n_tokens(vocab), params.logit_bias.size(), params.logit_bias.data()));
    }

    if (backend_greedy_fast) {
        samplers.push_back(llama_sampler_init_greedy_masked(greedy_mask));
    ...
    auto * result = new common_sampler {
        ...
        /* .cur     = */ {},
        /* .cur_p   = */ {},
        /* .backend_greedy_fast = */ backend_greedy_fast,
    };
...
struct common_sampler * common_sampler_clone(common_sampler * gsmpl) {
    return new common_sampler {
        ...
        /* .cur     = */ gsmpl->cur,
        /* .cur_p   = */ gsmpl->cur_p,
        /* .backend_greedy_fast = */ gsmpl->backend_greedy_fast,
    };
}
